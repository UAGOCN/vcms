var SAN = {
	...
	'UpDown': {
		'Vod': function($ajaxurl) {
			...
			$('.Up').click(function(){
				SAN.UpDown.Ajax($ajaxurl,'vod','up');
			})
			$('.Down').click(function(){
				SAN.UpDown.Ajax($ajaxurl,'vod','down');
			})
		},
		'News': function($ajaxurl) {
			if($("#Digup").length || $("#Digdown").length){
				...
				SAN.UpDown.Show($("#Digup_val").html()+':'+$("#Digdown_val").html(),'news');
			}
			$('.Digup').click(function(){
				SAN.UpDown.Ajax($ajaxurl,'news','up');
			})
			$('.Digdown').click(function(){					
				SAN.UpDown.Ajax($ajaxurl,'news','down');
			})
		},		
			...
			$.ajax({
				...
				success:function($html){
					if(!$html.status){
						...
						SAN.UpDown.Show($html.data,$model);
		...
		'Post': function(){
			...
			$.ajax({
				...
				success:function($string){
					if($string.status == 1){
						SAN.Comment.Show(Root+"index.php?s=Cm-Show-sid-"+Sid+"-id-"+Id+"-p-1");
					}
	...
	'Gold': {
		'Default': function($ajaxurl){
			if($("#Gold").length>0 || $("#Goldnum").length>0){
				$.ajax({
					...
					success: function($html){
						SAN.Gold.Show($ajaxurl,$html.data,'');
					}
				});			
			}else{
				if($(".Gold").length>0 || $(".Goldnum").length>0){
					SAN.Gold.Show($ajaxurl,$(".Goldnum").html()+':'+$(".Golder").html(),'');
				}
			}
		},
		'Show': function($ajaxurl,$html,$status){
			...
			//展示星级>评分>评分人
			$(".Gold").html(SAN.Gold.List($html.split(':')[0]));
			$(".Goldnum").html($html.split(':')[0]);
			$(".Golder").html(' 分 ('+$html.split(':')[1]+'人评价过此片)');
			//鼠标划过
			$(".Gold>span").mouseover(function(){
				$id = $(this).attr('id')*1;
				$(".Goldtitle").html(SAN.Gold.Title($id*2));
				$(".Goldtitle").show();
				$(".Goldtitle").css({display:'inline'});
				//刷新星级图标
				...
				$.ajax({
					...
					success: function($html){
						if(!$html.status){
							...
							SAN.Gold.Show($ajaxurl,$html.data,'onclick');
						}
					}
				});
			});
	...
	//图片延时加载 SAN.Lazyload.Box('frame'); <img class="lazy" data-original="{$ppvod.vod_picurl}" src="/images/blank.jpg" alt="xx" />
	'Lazyload':{
		'Show': function(){
	...
	//搜索联想 SAN.Suggest.Show('wd',10,'index.php?s=plus-search-index','index.php?s=vod-search-wd-');
	'Suggest': {
	...
	//Cookie SAN.Cookie.Set(name,value,days);
	'Cookie': {
	...
	//观看历史记录 SAN.History.Insert('硝烟背后的战争','',10,1,'','')
	'History': {
		'Json': '',
		'Display': true,
		'List': function($id){
		...
		'Clear': function($id){
			SAN.Cookie.Del('SAN_Cookie');
			$('#'+$id).append("<p style='padding: 80px 0; text-align: center'>已清空观看记录。</p>");
		},	
		'Show': function($id){
			$('#'+$id).show();
		},
		...
		'FlagHide': function($id){
			$('#'+$id).hover(function(){
				SAN.History.Display = false;
				SAN.History.Show($id);
			}, function(){
				SAN.History.Display = true;
				SAN.History.Hide($id);
			});
			if(SAN.History.Display){
				SAN.History.Hide($id);
			}
		},
		'Create': function($id){
			...
			if(this.Json){
				...
				var jsonstr = SAN.Cookie.Get('SAN_Cookie');
			...
			if(jsondata.length > 0){
				for($i=0; $i<jsondata.length; $i++){
					html += "<li class='top-line'><a href='"+jsondata[$i].vodlink+"' title='"+jsondata[$i].vodname+"'><span class='pull-right text-red'>"+jsondata[$i].vodcontinu+"</span>"+jsondata[$i].vodname+"</a></li>";
				}
			}else{
		...
		'Insert': function(vodname,vodlink,limit,days,vodcontinu){
			var jsondata = SAN.Cookie.Get('SAN_Cookie');
			if(jsondata != undefined){
				...
				jsonstr = '{video:[{"vodname":"'+vodname+'","vodlink":"'+vodlink+'","vodcontinu":"'+vodcontinu+'"},';
				for($i=0; $i<=limit; $i++){
					if(this.Json[$i]){
						jsonstr += '{"vodname":"'+this.Json[$i].vodname+'","vodlink":"'+this.Json[$i].vodlink+'","vodcontinu":"'+this.Json[$i].vodcontinu+'"},';
					}else{
						break;//continue;
					}
				}
			...
			SAN.Cookie.Set('SAN_Cookie',jsonstr,days);
		}
	}	
}
// 栏目页分页跳转
...
$(document).ready(function(){
	...
	SAN.Lazyload.Show();	
	//搜索联想
	SAN.Suggest.Show('wd',12,Root+'index.php?s=plus-search-vod',Root+'index.php?s=vod-search-wd-');
	//历史记录
	SAN.History.List('history');
	//影视顶踩初始化
	SAN.UpDown.Vod(Root+'index.php?s=Updown-'+SAN.Home.Channel+'-id-'+Id);
	//小说顶踩初始化
	SAN.UpDown.News(Root+'index.php?s=Updown-'+SAN.Home.Channel+'-id-'+Id);	
	//评论初始化
	SAN.Comment.Show(Root+"index.php?s=Cm-Show-sid-"+Sid+"-id-"+Id+"-p-1");
	//积分初始化
	SAN.Gold.Default(Root+'index.php?s=Gold-'+SAN.Home.Channel+'-id-'+Id);	
});