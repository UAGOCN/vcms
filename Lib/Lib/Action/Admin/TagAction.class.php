    ...
    public function show(){
        ...
        if(!empty($array)) {
            foreach($array as $key=>$val){
                $array[$key]['tag_url'] = U('Admin-'.ucfirst(getsidname($array[$key]['tag_sid'])).'/Show',array('tag'=>rawurlencode($array[$key]['tag_name'])),'',false,true);
            }
        }
		$this->assign($admin);
    ...
    public function showajax(){
		...
		/*foreach($array as $key=>$val){
			if($array[$key]['tag_sid']==2){
				$array[$key]['tag_url']=UU('Admin-News/Show',array('tag_name'=>rawurlencode($array[$key]['tag_name']),'tag_type'=>2),'',false,true);
			}else{
				$array[$key]['tag_url']=UU('Admin-Vod/Show',array('tag_name'=>rawurlencode($array[$key]['tag_name']),'tag_type'=>1),'',false,true);
			}
		}*/	
