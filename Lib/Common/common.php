...
function get_http_url(){
	return htmlspecialchars("https://".$_SERVER['HTTP_HOST'].$_SERVER["REQUEST_URI"],ENT_QUOTES);
}
//获取首页地址栏URL
function get_index_url(){
	return htmlspecialchars("https://".$_SERVER['HTTP_HOST'],ENT_QUOTES);
}
//获取根目录路径
...
function ff_get_hits($sidname,$type='hits',$array,$js=true){
	if((C('url_html') && $js) || $type=='insert'){
		return '<script src="'.C('site_path').'index.php?s=hits-show-id-'.$array[$sidname.'_id'].'-type-'.$type.'-sid-'.$sidname.'"></script>';
	}else{
		return $array[$type];
	}
}
...
// 获取广告调用地址
function getadsurl($str){
	return '<script src="'.'//'.parse_url(C('site_url'))['host'].C('site_path').C('admin_ads_file').'/'.$str.'.js"></script>';
}
/*******************替换自定义路径的变量****************************/
