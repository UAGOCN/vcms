...
function cookie($name, $value='', $option=null) {
        ...
        if (!empty($prefix)) {// 如果前缀为空字符串将不作处理直接返回
            foreach ($_COOKIE as $key => $val) {
                if (0 === stripos($key, $prefix)) {
                    $arr_cookie_options = array ('expires' => time() - 3600,'path' => $config['path'],'domain' => $config['domain'],'secure' => TRUE,'httponly' => TRUE,'samesite' => 'Lax');
    ...
    if ('' === $value) {
        ...
        if (is_null($value)) {
            ...
            $arr_cookie_options = array ('expires' => $expire,'path' => $config['path'],'domain' => $config['domain'],'secure' => TRUE,'httponly' => TRUE,'samesite' => 'Lax');
